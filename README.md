...
Версия политики: `2026.08.14.1`.

Исходная политика SHA-256: `cfe1d7c3e233fc7c3d328622375fa870db811e1eb3e6eb4f85a84fc677c6bbe9`.
...
из серверной блокировки: сейчас `api.oneme.ru` идёт напрямую, чтобы MAX работал
на устройстве, хотя VPN-сервер продолжает отклонять этот адрес. Затем локально
блокируются остальные подтверждённые каналы VPN-детектирования, государственные
назначения и P2P.
