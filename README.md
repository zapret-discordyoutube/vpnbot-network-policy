...
приватному компоненту `node-services/network-policy` и только генерируют этот
публичный клиентский снимок.

Публичный `policy.json` — также сгенерированная read-only копия точного
канонического поколения для аудита. Его наличие не создаёт второй источник
истины. Любая ручная правка файлов в `https://git.zapret.moe/zapretkvn/vpnbot-network-policy` будет перезаписана
следующей штатной генерацией.
